                ...
                if response and payload.lower() in response.text.lower():
                    self.add_vulnerability({
                        ...
                        "title": self.t(
                            ...
                            f"Уязвимость Reflected XSS в параметре '{param}'"
                        ),
                        "description": self.t(
                            f"XSS payload was reflected in the response via the '{param}' parameter.",
                            f"XSS payload reflected e response-um '{param}' parametri mjocov.",
                            f"XSS-нагрузка отразилась в ответе через параметр '{param}'."
                        ),
                        "url": test_url,
                        "parameter": param,
                        "method": "GET",
                        ...
                        "evidence": self.t(
                            ...
                            f"Payload haytnaberved response-um: {payload[:50]}",
                            f"Нагрузка найдена в ответе: {payload[:50]}"
                        ),
                        "recommendation": self.t(
                            ...
                            "Экранируйте все пользовательские данные. Внедрите заголовки CSP."
                        ),
                    ...
                    if response and payload.lower() in response.text.lower():
                        self.add_vulnerability({
                            ...
                            "title": self.t(
                                ...
                                f"Reflected XSS խocelilutyan form-i '{field}' dastum",
                                f"Уязвимость Reflected XSS в поле формы '{field}'"
                            ),
                            "description": self.t(
                                f"XSS payload was reflected in the response via form field '{field}' ({method.upper()}).",
                                f"XSS payload reflected e response-um form-i '{field}' dasti mjocov ({method.upper()}).",
                                f"XSS-нагрузка отразилась в ответе через поле формы '{field}' ({method.upper()})."
                            ),
                            "url": form_url,
                            "parameter": field,
                            "method": method.upper(),
                            ...
                            "evidence": self.t(
                                ...
                                "Payload haytnaberved response-um",
                                "Нагрузка найдена в ответе"
                            ),
                            "recommendation": self.t(
                                "Escape all user inputs. Implement CSP headers.",
                                "Escape arek bolor user inputs-y. Implement arek CSP headers.",
